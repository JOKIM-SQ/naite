...
## 2026-09-17 전환 당시 확인 결과와 상태

아래 설정 대기·차단 상태와 구현 범위는 전환 당시 기록이다. 이후 적용 여부와 실서비스 확인 범위는 문서 하단의 날짜별 후속 확인을 따른다.
...
사용자가 실제 영수증의 분석 결과 화면을 제공했다. REST에서 user_id 컬럼 조회 200으로 마이그레이션 적용을 확인했고, Vercel Production에 ANTHROPIC_API_KEY 등록도 확인했다. 위 SQL/키 미설정 기록은 이전 시점의 상태다. 당시 사용자 제공 화면에서는 상호·날짜·총액이 맞고 품목 누락·안내문 혼입이 보인다고 기록했다. 이는 당시 화면 관찰이며 실제 3장 정확도 N/3은 그 시점에 실측하지 않았다. 실제 Google 계정 입력 과정 자체를 에이전트가 관찰한 것은 아니다.

## 2026-09-17–18 실서비스 후속 확인

인앱 브라우저에서 공개 앱의 Google 로그인 버튼을 누른 뒤 기존 Google 세션으로 앱에 복귀하고 저장된 영수증 6건이 표시되는 것을 직접 확인했다. 새로고침 후 로그인·6건·기존 수정값도 복원됐다. 새 계정 생성·비밀번호 입력·최초 동의 과정은 이번 검증 범위에 포함하지 않았다.

표시 순서의 첫 3건을 원본을 열기 전에 표본으로 고정한 뒤 최초 추출의 날짜와 최종 결제금액을 대조했다. 두 값이 모두 맞은 결과는 **1/3**이다. A는 통과, B는 날짜 하루 오류, C는 최초 금액 오류로 실패했다. C의 현재 금액은 이미 수정됐으며 원본 연도 판독은 보류했다. 기존 결과의 사후 검증으로, 최초 업로드 당시 모델·프롬프트 버전은 화면에서 확인하지 못했으므로 현재 프롬프트의 신규 실행 성능으로 해석하지 않는다.

외식 필터 5건·장보기 필터 0건과 빈 상태, 통화별 합계·평균과 통화 미확인 금액의 별도 집계를 확인했다. 신규 업로드·수정·삭제는 이번에 수행하지 않았다. 기존 업로드의 사람 손 횟수, 셋업 → 첫 저장·최초 배포·첫 로딩 시간은 미측정이며 문서 품질·AI 친숙도·스택 적합성 등급은 미평가다. 공개 문서에는 익명화한 판정만 기록하고 거래값·상호·개인 원본·서명 URL은 포함하지 않는다. 상세 기준은 `SUBMISSION.md`, 실서비스 관찰 근거는 `evidence/production-verification.md`에 기록했다.
