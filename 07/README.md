...
- 품목 프롬프트는 구매 상품과 할인·쿠폰·세금·보증금·안내문을 구분하고 원문 약어를 보존하도록 지시한다. 실제 정확도 개선은 별도 비교로 확인하며, 기존 저장 결과에는 소급 적용하지 않는다.
- 날짜 선택기·총액·품목을 수정하면 자동 저장. 금액은 소계·세금이 아니라 최종 결제금액. 사용자 요청에 따라 통화 입력·요약·최초 추출값의 통화 표시는 제거했다. 기존 통화 데이터는 원본 보존과 저장 호환성을 위해 유지한다.
