# S07 Paper trail · 영수증 정리기

Supabase Storage에 원본을 저장하고 Claude Haiku 4.5로 상호·날짜·최종 결제금액과 지출 카테고리를 정리하는 대시보드 앱. Google 로그인으로 계정별 영수증을 보관하고 다른 브라우저에서도 이어서 정리한다.

## 실행

1. 기존 Supabase 프로젝트의 SQL Editor에서 새 설치는 `SUPABASE.sql`, 기존 설치는 `SUPABASE-OAUTH-MIGRATION.sql` 및 `SUPABASE-DELETE-MIGRATION.sql`을 적용한다. 기존 익명 영수증은 삭제하거나 자동 귀속하지 않는다.
2. `.env.example`을 참고해 `07/.env.local`에 실제 네 값을 설정한다. `[SENSITIVE]`는 키가 아니다. 키를 Git이나 대화에 올리지 않는다.
3. Supabase Auth의 Google 제공자를 활성화한다. Google 클라이언트의 반환 URI는 `https://kmfoeoxvsadlurpmkqwh.supabase.co/auth/v1/callback`. Supabase Redirect URLs에 공개 앱 `/`와 로컬 `http://127.0.0.1:3070/`를 추가한다. 기존 다른 앱 설정은 유지한다.
4. `cd 07 && npm ci && npm run dev`, http://127.0.0.1:3070 열기.
5. `npx tsc --noEmit`, `npm run lint`, `npm test`, `npm run build` 실행.

TypeScript 명령은 기존 JavaScript를 emit 없이 컴파일 확인한다. checkJs는 끄며 엄격한 타입 검증을 했다고 주장하지 않는다. 의미 있는 동작 경계는 node:test로 검증한다.

## 사용

- JPEG/PNG/WebP, 장당 3MiB 이하를 한 번에 최대 3장 선택.
- 원본 저장 후 자동 분석. 읽을 수 없는 값은 비워 둔다.
- AI가 가게의 성격을 기준으로 쇼핑·장보기·외식·그 외 중 하나를 배정한다. 구매 품목은 더 이상 추출하지 않는다. 분류 근거가 부족하거나 기존 기록에 분류가 없으면 그 외로 표시하며 직접 수정할 수 있다.
- 영수증 카드의 편집 창에서 상호·날짜·총액·카테고리를 수정하면 자동 저장한다. 통화 입력은 없으며 내부 통화 데이터는 서로 다른 단위의 금액을 잘못 합산하지 않도록 집계에 사용한다.
- `수정한 필드`는 서버에 저장된 최상위 필드 변경의 누적 수이며 실제 클릭 수와 다르다.
- 최초 추출 결과는 보존. `N/3 정확도`는 원본과 대조해 따로 기록한다. 모델 자체 확신을 정확도로 쓰지 않는다.
- 같은 Google 계정으로 로그인하면 전체 기록을 페이지별로 복원한다. 로그아웃하면 화면의 기록과 대기 중인 요청을 정리한다.

## 배포와 제출

공개 사이트: https://s07-receipt-organizer.vercel.app

소스: https://github.com/JOKIM-SQ/naite/tree/codex/s07-receipt-organizer/07

2026-09-17에 네 카테고리·영수증 카드·지출 대시보드를 production에 배포했다(`4a93041`, READY). 배포 및 로컬 검증은 `evidence/dashboard-review.md`에 기록했다. 후속 카드 표시·카테고리별 pill 색상 개선은 `evidence/card-polish-review.md`에서 검증과 배포 결과를 확인한다.

2026-09-17–18 인앱 브라우저에서 공개 앱의 Google 버튼을 누른 뒤 기존 Google 세션으로 앱에 복귀하고 저장 기록 6건을 확인했다. 새 계정 생성·비밀번호 입력·최초 동의 과정은 검증하지 않았다. `AUTH.md`에는 당시 외부 설정 대기 기록과 날짜별 후속 확인을 구분해 남겼다.

원본을 열기 전에 표시 순서의 첫 3건을 고정하고 최초 추출의 날짜·최종 결제금액을 대조한 결과는 **N/3 = 1/3**이다. 두 값이 모두 맞아야 통과하며 A는 통과, B는 날짜 하루 오류, C는 최초 금액 오류로 실패했다. C의 현재 금액은 이미 수정되어 있고 원본 연도 판독은 보류했다. 기존 결과의 사후 검증으로, 현재 프롬프트의 신규 실행 성능을 나타내지 않는다. 익명화한 판정과 측정 기준은 `SUBMISSION.md`에 기록했다.

외식 필터 5건·장보기 필터 0건과 빈 상태, 통화별 합계·평균 및 통화 미확인 금액의 별도 집계, 새로고침 후 로그인·6건·기존 수정값 복원을 확인했다. 이번에는 신규 업로드·수정·삭제를 하지 않았다. 업로드 당시 사람 손 횟수, 셋업 → 첫 저장·최초 배포·첫 로딩 시간은 **미측정**이며 문서 품질·AI 친숙도·스택 적합성 등급은 **미평가**다. 실서비스 검증 범위는 `evidence/production-verification.md`에서 확인한다.

- 전용 Vercel 프로젝트 `s07-receipt-organizer`. Git 연결 시 Root Directory `07`, Build `npm run build`, Output `public`.
- 서버 환경변수 `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `ANTHROPIC_API_KEY`; 브라우저 Auth 공개키 `SUPABASE_PUBLISHABLE_KEY`; 모델 선택 시 `ANTHROPIC_MODEL`.
- Git 자동배포 연결 시 Ignored Build Step `git diff --quiet HEAD^ HEAD -- .`로 주차별 변경만 빌드.
- `/docs/plan.html`, `/docs/report.html`은 팀 템플릿 CSS·data-f 규약을 유지한다.
- 공식 현재 가이드: 목 18:00 갤러리에 URL 제출, 금 09:30 데모. S07은 9월 17일 제출, 9월 18일 데모. 공통 원본 템플릿은 그대로 두고 S07 문서의 마감 문구만 현재 가이드에 맞췄다. 레이아웃·CSS·data-f는 유지한다.
- 갤러리 드랍/스택 비교표 게시 여부는 실제 완료 후 기록한다. 사이트 배포만으로 제출 완료라고 하지 않는다.
- 붙여넣을 URL·메타데이터·레지스트리 TSV·3분 데모·남은 측정표는 `SUBMISSION.md`에 준비했다. initial-b 실제 등록과 스택 리포트 게시는 아직 하지 않았다.

## 3분 데모 순서

1. Google 로그인 후 사진 3장을 한 번에 선택한다.
2. 자동으로 채워진 상호·날짜·총액·카테고리와 원본을 비교한다.
3. 틀린 값이 있으면 수정하고 저장 표시를 확인한다.
4. 카테고리 필터와 대시보드 집계를 확인하고, 새로고침해 수정값과 원본이 유지되는지 확인한다.
5. 원본과 대조한 최초 추출 정확도 N/3, 실제 센 업로드 후 사람 손 횟수, 스택 리포트를 보여준다. 기존 3건의 사후 대조는 1/3이며 새 업로드의 결과와 구분한다. 기존 업로드의 사람 손 횟수는 미측정이므로 새 시연에서 별도로 세고 누적 수정 필드 수로 대신하지 않는다.

## 제외 범위

별도 이메일·비밀번호 회원가입 / 관리자 페이지 / CSV 다운로드·회계 연동.

## 화면과 사용 흐름

S04·S05·S06의 시각 구성과 상태 피드백을 참고한 Paper trail 디자인. 업로드 가까이 연결 상태를 표시하고, 사진 선택 후 첫 결과로 이동한다. 사이드바로 카테고리를 고르고 월·상호 검색으로 기록을 좁힌다. 영수증은 사진과 핵심 정보가 있는 카드로 표시하고, 편집 창에서 원본과 함께 수정한다. 삭제 확인 뒤 원본 사진과 저장 행을 함께 정리하며, 정리 실패 시 재시도할 수 있다. 대시보드는 저장된 수정값을 기준으로 KPI와 월별·카테고리별 지출을 계산한다. 금액 미확인은 합계에서, 날짜 미확인은 월별 차트에서 제외하고 서로 다른 통화는 합산하지 않는다. 디자인 기준은 `DESIGN.md`, 기존 화면 QA는 `evidence/dashboard-review.md`와 `evidence/card-polish-review.md`, 후속 실서비스 관찰은 `evidence/production-verification.md`에 기록한다.

메인의 영수증 분석 미리보기는 화면에 보이는 동안 자동으로 반복 재생한다. 화면 밖이나 숨긴 탭에서는 정지하고, 움직임 줄이기 설정에서는 완성된 그림과 안내만 표시한다. 실제 OCR 결과나 진행률을 나타내지 않는다. 파비콘·192/512px 앱 아이콘·180px Apple 홈 화면 아이콘과 웹 매니페스트를 제공한다. 최초 검증은 `evidence/animation-review.md`, 반복 재생·공개 배포 결과는 `evidence/loop-release.md`에 기록한다.

## 외부 서비스 없이 화면 회귀 검증

`node scripts/qa-server.mjs`는 3071 포트의 **테스트 전용 서버**다. 실제 API·UI·검증·계정 소유권·저장 충돌 코드를 사용하되 Supabase/Claude HTTP 응답은 테스트 fixture로 대체한다. Google 버튼은 이 서버에서만 로컬 PKCE 인증 fixture를 사용하며 실제 Google 로그인의 근거가 아니다. 이 결과는 실제 OCR 정확도나 외부 저장 성공의 증거가 아니다. 이 파일과 fixture는 Vercel 배포에서 제외한다.
