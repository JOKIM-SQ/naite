...
test('Claude JSON 응답을 전체 분석과 리뷰별 근거로 정규화한다', () => {
  const analysis = parseStructuredAnalysis(`\`\`\`json
  {"summary":"가성비는 좋지만 첫 사용이 어렵습니다.","positiveFactors":["배송이 빠릅니다","가격이 합리적입니다"],"negativeFactors":["설치 안내가 부족합니다"],"painPoints":["초기 설정에서 멈춥니다"],"recommendedFocus":"설치 안내를 첫 화면에 추가하세요.","reviewSignals":[{"reviewIndex":1,"positiveFactors":["배송이 빠릅니다"],"negativeFactors":["설치 안내가 부족합니다"],"painPoints":["초기 설정에서 멈춥니다"]},{"reviewIndex":2,"positiveFactors":["가격이 합리적입니다"],"negativeFactors":[],"painPoints":[]}]}
  \`\`\``);
  ...
  assert.deepEqual(analysis, {
    ...
    reviewSignals: [
      { reviewIndex: 1, positiveFactors: ['배송이 빠릅니다'], negativeFactors: ['설치 안내가 부족합니다'], painPoints: ['초기 설정에서 멈춥니다'] },
...
test('Claude 분석은 요청한 모든 리뷰에 대응하는 근거를 반환해야 한다', () => {
  assert.throws(() => parseStructuredAnalysis(
    '{"summary":"요약","positiveFactors":[],"negativeFactors":[],"painPoints":[],"recommendedFocus":"개선","reviewSignals":[{"reviewIndex":1,"positiveFactors":[],"negativeFactors":[],"painPoints":[]}]}'
  , 2), /모든 리뷰/);
});
  ...
  const result = await analyzeReviews({
    ...
    fetchImpl: async (url, options) => {
      ...
      return new Response(JSON.stringify({
        content: [{ type: 'text', text: '{"summary":"요약","positiveFactors":["장점"],"negativeFactors":["단점"],"painPoints":["불편"],"recommendedFocus":"개선","reviewSignals":[{"reviewIndex":1,"positiveFactors":["장점"],"negativeFactors":["단점"],"painPoints":["불편"]},{"reviewIndex":2,"positiveFactors":[],"negativeFactors":[],"painPoints":[]}]}' }],
      }), { status: 200 });
    },
  });
